...
const dbCheckGeojsons = async user => {
  ...
  await Promise.all(user.geojsons.map(async geoID => { // loop through all the geojsons in user.
    const geojson = await Geojson.findById(geoID).populate() // try and find that geojson in the db.

    if (!geojson) { // If that geojson doesn't exist in the db.
      newGeos = newGeos.filter(g => g !== geoID) // Remove that geojson from user.geojsons and save the new arr in newGeos.
